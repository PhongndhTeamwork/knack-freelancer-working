...
export const FeedbackDialog = ({setOpen, setMessage, setTriggerNotice, triggerNotice, feedback}: Props) => {
    ...
    const validate = () => {
        if(!feedbackInfo.star || !feedbackInfo.customerPosition || !feedbackInfo.companyName || !feedbackInfo.customerName || !feedbackInfo.comment) {
            setMessage({
    ...
    return (
        <div className="bg-white px-3 pb-6">
                ...
                <div className="p-4 pt-0 space-y-4 text-[#545454]">
                    <div className="space-y-4">
                        ...
                        <div className="space-y-2">
                            ...
                            <CustomTextarea value={feedbackInfo.comment} placeholder="" className="responsive-text-16" onChange={(value) => {
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div className="space-y-2">
                                ...
                                <Input placeholder="" value={feedbackInfo.customerName} className="responsive-text-16 h-11" onChange={(e) => {
                                    setFeedbackInfo((prev) => ({...prev, customerName: e.target.value}))
                                }}/>
                            </div>
                            <div className="space-y-2">
                                <Label className="responsive-text-16">Công ty</Label>
                                <Input value={feedbackInfo.companyName} placeholder="" className="responsive-text-16 h-11" onChange={(e) => {
                                    setFeedbackInfo((prev) => ({...prev, companyName: e.target.value}))
                                }}/>
                            </div>
                            <div className="space-y-2">
                                <Label className="responsive-text-16">Chức danh tại công ty</Label>
                                <Input value={feedbackInfo.customerPosition} placeholder="" className="responsive-text-16 h-11" onChange={(e) => {
            ...
            <div className="flex justify-end mt-6 gap-4">
                {!feedback && <Button disabled={isLoading} variant="dark" onClick={handleCreate}>{isLoading && <CustomSpinner size="sm"/>} Thêm feedback</Button>}
                {feedback && <Button disabled={isLoading} variant="dark" onClick={handleUpdate}>{isLoading && <CustomSpinner size="sm"/>} Lưu thay đổi</Button>}
            </div>
        </div>
